...
# End marker is right before `setTimeout` in openDetail
end_pos = c.find("setTimeout(() => {\n        const container = document.getElementById(`mv-container-${p.id}`);", start_pos)
if end_pos == -1:
    print("Could not find end_pos")
    exit(1)

replacement_code = """const scopeBadges = [
        'Structural Steel Detailing',
        'Connection Design',
      ...
      const hasMediaHeader = Boolean(p.modelUrl || p.video);
...
c = c[:start_pos] + replacement_code + c[end_pos:]

with open('js/app.js', 'w', encoding='utf-8') as f:
    f.write(c)

print("Clean replacement complete!")
