...
// 1. Remove the old skyline HTML
content = content.replace(/<div class="loader-skyline">[\s\S]*?<\/div>\s*<\/div>/, "");

// 2. Remove the wavy SVG HTML
content = content.replace(/<svg viewBox="0 0 280 80" aria-hidden="true">[\s\S]*?<\/svg>/, "");

// 3. Update the loader CSS to be clean and cinematic
...
const newLoaderCss = `  #loaderLogo {
  ...
  @keyframes logoBreathe {
    0% { transform: scale(1) translateY(0); filter: drop-shadow(0 10px 20px rgba(0,0,0,0.08)); }
    100% { transform: scale(1.05) translateY(-4px); filter: drop-shadow(0 15px 30px rgba(0,0,0,0.15)); }
  }`;

// Carefully replace block
const startIdx = content.indexOf("  #loaderLogo {");
const endIdx = content.indexOf("/* ================= APP LAYOUT ================= */");
if (startIdx !== -1 && endIdx !== -1) {
  content = content.substring(0, startIdx) + newLoaderCss + "\n  " + content.substring(endIdx);
}
...
content = content.replace(oldJsTarget, newJsTarget);

// Add progress bar HTML if not exist
if (!content.includes('ldr-progress-bar')) {
  content = content.replace('<div class="ldr-caption">INITIALIZING PLATFORM <span id="ldrPct">0%</span></div>', 
    '<div class="ldr-caption">LOADING PORTAL <span id="ldrPct">0%</span></div>\n      <div class="ldr-progress-bar"><div class="ldr-progress-fill" id="ldrFill"></div></div>');
}

// Update JS for progress fill
content = content.replace("if(pct) pct.textContent = _loaderProgress + '%';", "if(pct) pct.textContent = _loaderProgress + '%';\n          const fill = document.getElementById('ldrFill');\n          if (fill) fill.style.width = _loaderProgress + '%';");

content = content.replace(/v=\d+/g, "v=" + Date.now());
fs.writeFileSync(p, content);
console.log("Upgraded loader to premium cinematic style");
